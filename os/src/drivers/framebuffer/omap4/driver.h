...
class Framebuffer::Driver
{
	...
	private:
		...
		bool init_lcd(addr_t phys_base);
		
		bool init_hdmi(addr_t phys_base);
...
bool Framebuffer::Driver::init_lcd(Framebuffer::addr_t phys_base)
{
	...
	/*
	if (!_dispc.wait_for<Dispc::Control1::Go_lcd>(
		Dispc::Control1::Go_lcd::HW_UPDATE_DONE, _delayer))
	{
...
bool Framebuffer::Driver::init_hdmi(Framebuffer::addr_t phys_base)
{
                               ...
                               Framebuffer::addr_t phys_base)
{
	...
	switch (output) {
		case OUTPUT_LCD:
			ret = init_lcd(phys_base);
			break;
		case OUTPUT_HDMI:
			ret = init_hdmi(phys_base);
			break;
		default:
